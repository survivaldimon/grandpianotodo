...
- **`SESSION_2026_01_07_SCHEDULE_CONFLICTS.md`** — проверка конфликтов постоянного расписания:
  - Исправление RenderFlex ошибки в диалоге постоянного расписания
  - Секция "Время занятий" для повторяющихся занятий (разное время для каждого дня)
...
2. **Конфликт с обычными занятиями** (`hasLessonConflictForDayOfWeek`)
   - Таблица: `lessons`
   - Проверяет ВСЕ будущие занятия (не только N недель)
   - Исключает занятия того же ученика

**Метод проверки всех будущих занятий:**
```dart
/// Проверить конфликт с ВСЕМИ будущими занятиями для дня недели
Future<bool> hasLessonConflictForDayOfWeek({
  ...
  // Загружает все будущие занятия в кабинете с пересечением времени
  // Фильтрует по дню недели на клиенте
}
```

**UI индикация конфликтов:**
- Конфликтующие дни подсвечиваются красным цветом
- Иконка предупреждения рядом с днём
...
### 49. Объединённая система бронирований (Unified Bookings)
