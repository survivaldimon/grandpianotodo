  ...
  routes: [
    ...
    ShellRoute(
      ...
      routes: [
        ...
        // Rooms (настройки кабинетов)
        GoRoute(
          path: '/i/:institutionId/rooms',
          builder: (_, state) => RoomsScreen(
            institutionId: state.pathParameters['institutionId']!,
          ),
        ),

        // Schedule (основное расписание всех кабинетов)
        GoRoute(
          path: '/i/:institutionId/schedule',
          builder: (_, state) => AllRoomsScheduleScreen(
            institutionId: state.pathParameters['institutionId']!,
          ),
        ),
...
#### RoomsScreen (Кабинеты) — Настройки → Кабинеты
...
**Действия:**
- Tap на карточку кабинета → открывается форма редактирования (BottomSheet)
- [+] FAB → создание нового кабинета
- [≡] → режим сортировки (стрелки вверх/вниз)
- (⋮) меню → "Редактировать" или "Удалить"

**Файл:** `lib/features/rooms/screens/rooms_screen.dart`

**Примечание:** Расписание кабинетов доступно через основной экран расписания (вкладка "Расписание").

---

#### AllRoomsScheduleScreen (Расписание всех кабинетов)

**Режимы отображения:**
- **Дневной** (по умолчанию) — один день, все кабинеты в колонках
- **Недельный** — неделя, один выбранный кабинет

**Layout (дневной режим):**
```
┌─────────────────────────────────────────────────────┐
│ Расписание                [День|Неделя] [📅] [+]   │
├─────────────────────────────────────────────────────┤
...
**Компоненты:**
- `WeekDaySelector` — горизонтальный скролл дней
- `TimeGrid` — вертикальная временная сетка
- `RoomHeaders` — шапка с номерами кабинетов
- `LessonBlock` — блок занятия (цвет = цвет предмета)
- `BookingBlock` — блок брони (оранжевый, иконка 🔒)

**Цвет блока занятия:**
- Есть предмет → цвет предмета
- Нет предмета → цвет типа занятия
- Ничего → primary цвет

**Действия:**
- Tap на занятие → LessonDetailBottomSheet
- Tap на пустое место → CreateLessonBottomSheet
- [+] FAB → быстрое создание занятия
- [📅] → DatePickerDialog
- Long press на занятие → меню (редактировать/удалить)
...
**Фильтры:**
- Горизонтальные кнопки с прокруткой
...
**Выбор темы:** Настройки → "Тема оформления"

**Архитектура:**
```
lib/core/theme/
├── app_colors.dart      # Палитра (light + dark)
├── app_theme.dart       # lightTheme + darkTheme
└── theme_provider.dart  # Riverpod + SharedPreferences
```

**Важно для разработки:**
При создании UI использовать theme-aware цвета:
